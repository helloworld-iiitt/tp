            ...
            {quickLink1s.map((nav, index) => (
              <li key={index} className="md:mb-5">
                <a
                  ...
                  className="text-offwhite whitespace-nowrap text-sm font-normal justify-center md:justify-start mb-4 md:mb-6 space-links flex gap-2 items-center"
                >
                  {nav.icon} {nav.title}
                </a>
              </li>
  ...
  {navigation.map((nav, index) => (
    <li key={index} className="mb-2 md:mb-5">
      ...
      <div className="px-4 bg-gray-900 py-4 text-[0.75rem] text-white font-semibold">
        <h3 className="text-center text-offwhite">@2025 - All Rights Reserved</h3>
        <h3 className="text-center text-offwhite">
          🌟 Brought to You with Dedication by the TNP Cell, IIIT Trichy 🌟
        </h3>
      </div>
    </div>
  );
