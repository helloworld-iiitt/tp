...
export default function MemberCard({ idx, name, role, imageUrl, linkedinUrl, githubUrl, phone, email }: { idx: number, name: string, role: string, imageUrl: string, linkedinUrl: string, githubUrl?: string, phone: string, email: string }) {
    ...
    return (
        <Card
            ...
            className="text-center border-0 rounded-lg w-[300px] hover:text-white transition-all duration-300 border-b-4"
        ...
        >
            ...
            >
                <div className={`${styles.circle} ${isHovered ? styles.circlehover + " rounded-none object-fill m-0 border-none w-[160px] h-[160px]" : "m-[25px] border-4 shadow-lg border-white overflow-hidden object-fill h-[100px] w-[100px]"} relative`}
                >
                    <Image
            ...
            <CardFooter className={`flex flex-col justify-center items-center gap-2 text-[#808080] ${isHovered ? "text-white" : ""}`}>
