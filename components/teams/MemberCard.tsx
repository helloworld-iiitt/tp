...
export default function MemberCard({ idx, name, role, imageUrl, linkedinUrl, githubUrl, phone, email }: { idx: number, name: string, role: string, imageUrl: string, linkedinUrl: string, githubUrl: string, phone: string, email: string }) {
    ...
    return (
        <Card
            ...
            className="text-center border-0 rounded-lg w-[250px] hover:text-white transition-all duration-300 border-b-4"
        ...
        >
            ...
            >
                <Image
                    src={imageUrl || "/profile/pro-pic.jpeg"}
                    alt=""
                    width={100}
                    height={100}
                    className={`${styles.circle} ${isHovered ? styles.circlehover + " rounded-none object-fill m-0 border-none w-[150px]" : "m-[25px] border-4 shadow-lg border-white overflow-hidden object-cover"} `}
                />
            </CardHeader>
